...
# Cleaning and enrichment rules
def clean_name(name):
    name = re.sub(r'^(Prof\.?|Profa\.?|Dr\.?|Dra\.?|MSc|Ph\.?D\.?|\s+)+', '', name, flags=re.IGNORECASE)
    name = re.sub(r',?\s*(DSc|D\.Sc\.|Ph\.?D\.?|MSc|M\.Sc\.|Doutor|Doutora).*', '', name, flags=re.IGNORECASE)
    name = re.sub(r'Depto\..*', '', name)
    name = re.sub(r'\s+', ' ', name).strip()
    return name

# Specific high-value research group mappings
known_researchers = {
...
for rec in raw_records:
    name = clean_name(rec['nome'])
    if not name or len(name) < 5 or any(x in name.lower() for x in ['departamento', 'conselho', 'secretaria', 'faculdade', 'universidade', 'aluno', 'disciplinas', 'curso', 'pesquisa']):
        continue

    # Avoid duplicate entry by clean name
    ...
    # Standardize email fallback traceability
    email = rec.get('email', '')
